...
import { getDynamicFormConfig } from '@centopsmodule/shared/resources/api-constants';
import { Button, Label, Track } from '@centopsmodule/shared';
import type { ValidationRule } from './validation';
import { validateForm } from './validation';
import { DynamicInput } from './input';
import type { DynamicFormConfig, KeyValueMap, KeyValuesMap } from './types';
import { useTranslation } from 'react-i18next';
import { useToast } from '@centopsmodule/shared/hooks/useToast';
...
export const DynamicForm: FC<DynamicFormProps> = ({
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    if (onSubmit) {
      ...
      axios
        .post(formConfig.submitUrl, submitedForm)
        .then(() => {
          toast.open({
            type: 'success',
            title: t('forms.toast_title'),
  ...
  if (error) {
    return (
      <div>
        ...
        <Button appearance="primary" onClick={fetchFormConfig}>
          {t('forms.retry')}
        </Button>
      </div>
    );
  ...
  if (!formConfig) {
    return <div>{t('forms.loading')}</div>;
  }
