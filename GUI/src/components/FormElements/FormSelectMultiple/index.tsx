...
import 'components/FormElements/FormSelectMultiple/FormSelect.scss';
...
const FormSelectMultiple: FC<FormSelectMultipleProps> = ({
  ...
  // TODO, unused state
  const [_selectedStateItems, setSelectedStateItems] =
  ...
  useEffect(() => {
    setSelectedItems(
      options.filter((o) => defaultValue?.includes(String(o.value)))
    );
            ...
            options.map((item, index) => (
              <li
                className={clsx('select__option', {
                  'select__option--selected': selectedItems.find(
