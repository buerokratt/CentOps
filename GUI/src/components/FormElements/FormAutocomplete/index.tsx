...
export const FormAutocomplete: FC<FormAutocompleteProps> = ({
  ...
  const [isHideSuggestions, setIsHideSuggestions] = useState(
    shouldHideSuggestions
  );
  ...
  const onKeyUpHandler = (e: BaseSyntheticEvent<KeyboardEvent>) => {
    ...
    const newSuggestions = data.filter((item: string) =>
      item
        ...
        .includes(
          value.toLocaleLowerCase().replace(excludeCharacters ?? '', '')
        )
    );
    setSugesstions(newSuggestions);
    setIsHideSuggestions(
      newSuggestions.length > 0 && newSuggestions[0] === value
    );
  };
    ...
    <div className="sugesstion-auto">
      ...
      {suggestions.length > 0 && (
        <div
          ...
          style={{ display: isHideSuggestions ? 'none' : 'block' }}
        >
