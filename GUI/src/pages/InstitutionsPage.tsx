...
import { Button, DataTable, FormInput, Modal, Switch, Track } from 'components';
...
import {
  ...
  editInstitute,
  getInstitutions,
  toggleInstituteActiveStatus,
} from 'resources/api-constants';

const InstitutionsPages: FC = () => {
  const { t } = useTranslation();
  const toast = useToast();

  const [institutionToDelete, selectInstitutionIdToDelete] =
    useState<Institution | null>(null);
  const [newInstitute, setNewInstitute] = useState<{
    id?: number;
    name?: string;
    contactEmail?: string;
    guiStatus: 'none' | 'add' | 'edit';
  }>({ name: '', contactEmail: '', guiStatus: 'none' });
  ...
  const handleDelete = () => {
    ...
    axios
      .post(deleteInstitute(institutionToDelete.id), { withCredentials: true })
      .then(() => {
        refetch();
        toast.open({
          type: 'success',
  ...
  const showConfirmDeleteModal = (inst: Institution) =>
    selectInstitutionIdToDelete(inst);
  const closeConfirmDeleteModal = () => selectInstitutionIdToDelete(null);
  const toggleActiveStatus = (id: number) => {
    axios
      .post(toggleInstituteActiveStatus(id), { withCredentials: true })
      .then(() => {
        refetch();
        toast.open({
          type: 'success',
  ...
  const showEditModal = (inst: Institution) =>
    setNewInstitute({ guiStatus: 'edit', ...inst });
  ...
  return (
    <>
      ...
      <Track justify="end">
        <Button
          appearance="primary"
          onClick={() => setNewInstitute({ guiStatus: 'add' })}
        >
          Add a new institution
        </Button>
      </Track>
      <DataTable data={institutions ?? []} columns={institutionColumns} />
      {institutionToDelete && (
        <Modal
        ...
        >
          ...
          <Track justify="end" gap={12}>
            <Button
              ...
              onClick={() => setNewInstitute({ guiStatus: 'none' })}
            >
              Cancel
            </Button>
